...
class Brief(BaseModel):
    ...
    image_mode: Literal["editorial", "character"] = Field(default="editorial", description="editorial = museumsreifes Konzept-Plakat (Steffis Creative-Director-Prompt); character = Foto-Szene MIT Steffis Gesicht, für persönliche Beiträge/Wachstumsreihe")
    poster_briefing: str = Field(default="", description="Nur bei image_mode=editorial: das vollständig ausgefüllte PROJEKTBRIEFING nach Steffis Vorlage (siehe Systemprompt), auf Deutsch, als ein zusammenhängender Text mit allen Feldern")
    image_headline: str = Field(default="", description="Nur bei image_mode=character: die Kernaussage, max. 12 Wörter")
    image_body: str = Field(default="", description="Nur bei image_mode=character: 1–2 kurze Sätze plus optional eine Abschlussfrage")
    image_scene_prompt: str = Field(default="", description="Nur bei image_mode=character: englischer Prompt für die Foto-Szene MIT Steffi (Ort, Handlung, Licht — nicht ihr Aussehen, das kommt vom Referenzfoto)")
    image_query: str = Field(default="", description="Nur als Ersatz ohne API-Zugang: englische Suchwörter für ein Pexels-Hintergrundfoto")
